...
IPAddress mqtt_server = {192,168,0,1}; // MQTT Server IP
const char* mqtt_id = "";

const char* state_topic = "home/ws_esp32";
MQTT mqttClient(state_topic, mqtt_id, mqtt_server);

#define BME_SDA_PIN GPIO_NUM_12
#define BME_SCL_PIN GPIO_NUM_13
...
void draw_sens_battery(int x, int y, int percentage, float voltage);
...
void setup()
{
  ...
  if (SPIFFS.begin())
  {
    ...
    {
/* Init BME280 sensor */     
      log_i("Init BME280 sensor!");
      Wire.begin(BME_SDA_PIN, BME_SCL_PIN);
      while(!bme.begin())
      {
        log_i("Could not find BME280 sensor!");
        delay(1000);
      }
/* end Init BME280 sensor */     
...
/* Read BME280 sensor data */     
      ...
      log_i("Pres: %f  Temp: %f  Hum: %f", bme_pres, bme_temp, bme_hum);
      Wire.~TwoWire();
/* end Read BME280 sensor data */      
      ...
      else
      {
        if (start_WiFi() == WL_CONNECTED && setup_time() == true)
        {
          ...
          SensorData data;
          data.room_temp = bme_temp;
          data.room_humidity = bme_hum;
          data.room_pressure = bme_pres;
          data.outside_temp = ble_sens[0].temperature;
          data.outside_humidity = ble_sens[0].humidity;
...
void display_info()
{
  ...
  draw_sens_battery(150, 30, ble_sens[0].battery_level, ble_sens[0].battery_voltage);
}
...
void draw_thp_section(uint16_t x, uint16_t y) // temperature, humidity, pressure section
{
  int xOffset = 40;
  setFont(osans48b);
  drawString(x, y, String(ble_sens[0].temperature, 1) + " °C", CENTER);
...
//     uint16_t tmp_x = x;
//     tmp_x -= 140;
//     drawString(tmp_x, y, String(ble_sens_temp,1) + " °C", CENTER);
//     tmp_x = x;
//     tmp_x += 140;
//     drawString(tmp_x, y, String(ble_sens_humidity,1) + " %", CENTER);
//   } else {
//     uint16_t tmp_x = x;
//     tmp_x -= 140;
//     drawString(tmp_x, y, "---- °C", CENTER);
//     tmp_x = x;
//     tmp_x += 140;
//     drawString(tmp_x, y, "---- %", CENTER);
//  }
// end added for BLE
//y += osans8b.advance_y;
  ...
  setFont(osans32b);
//  int sw = drawString(x - xOffset, y, String(ble_sens[0].humidity, 1) + "%", RIGHT);
  int sw = drawString(x, y, String(ble_sens[0].humidity, 1) + "%", RIGHT);
...
void draw_sun_section(uint16_t x, uint16_t y)
{
  ...
  drawString(x - r - 20, y - 40, weather.forecast.sunrise, RIGHT);
  drawString(x + r + 20, y - 40, weather.forecast.sunset, LEFT);
//  draw_moon_section(x, y, "qq");
}
...
//  double Phase = NormalizedMoonPhase(dd, mm, yy);
  double Phase = (float) weather.forecast.moon_code / 16.;
  ...
  drawCircle(x + diameter - 1, y + diameter, diameter / 2, Black, false);
}

void draw_moon_section(uint16_t x, uint16_t y, String hemisphere)
{
  DrawMoonImage(x, y); // Different references!
  DrawMoon(x - 38, y - 38, 75/*, now_utc->tm_mday, now_utc->tm_mon + 1, now_utc->tm_year + 1900, Hemisphere*/); // Spaced at 1/2 moon size, so 10 - 75/2 = -28
}
...
void draw_thp_forecast_section(uint16_t x, uint16_t y, uint8_t part)
{
  ...
  setFont(osans16b);
  drawString(x, y + 100, String(weather.forecast.parts[part].pressure_mm), CENTER);

  setFont(osans8b);
  drawString(x, y + 125, "mm/Hg", CENTER);
}
...
//    drawStringWithLB(x + L_SIZE / 2, y + L_SIZE + 5, get_description_condition(weather.fact.condition), osans8b, CENTER);
    drawStringWithLB(x + L_SIZE / 2, y + L_SIZE - 5, get_description_condition(weather.fact.condition), osans10b, CENTER);
  }
  else
  {
    drawStringWithLB(x + 10, y + S_SIZE - 5, get_description_condition(weather.forecast.parts[forecast_part].condition), osans8b, LEFT);
    uint8_t prec_prob = weather.forecast.parts[forecast_part].prec_prob;
    drawString(x + 10, y + S_SIZE + 40, String(weather.forecast.parts[forecast_part].prec_mm, 1) + "mm    " + String(prec_prob) + "%", LEFT);
    // drawString(x + S_SIZE / 2 - 10, y + S_SIZE + 35, String(weather.forecast.parts[forecast_part].prec_mm, 1) + "mm", RIGHT);
    // drawString(x + S_SIZE / 2 + 10, y + S_SIZE + 35, String(prec_prob) + "%", LEFT);
  }
}
...
void display_forecast_weather()
{
  ...
  y = y + 35;
  setFont(osans32b);
  drawString(x, y, String(bme_temp, 1) + " °C", CENTER);
  int sw = drawString(x, y + 70, String(bme_hum, 1) + " %", CENTER);
...
bool decode_json(char *jsonStr, int size)
{
  ...
  weather.fact.condition = jo["fact"]["condition"].as<const char *>();
  weather.fact.daytime = jo["fact"]["daytime"].as<const char *>();
  weather.fact.feels_like = jo["fact"]["feels_like"].as<int8_t>();
  weather.fact.humidity = jo["fact"]["humidity"].as<uint8_t>();
  weather.fact.icon = jo["fact"]["icon"].as<const char *>();
  ...
  weather.fact.season = jo["fact"]["season"].as<const char *>();
  weather.fact.temp = jo["fact"]["temp"].as<int8_t>();
  weather.fact.temp_water = jo["fact"]["temp_water"].as<int8_t>();
  weather.fact.wind_dir = jo["fact"]["wind_dir"].as<const char *>();
  ...
  weather.forecast.moon_text = jo["forecast"]["moon_text"].as<const char *>();
  for (uint8_t i = 0; i < 2; i++)
  {
    weather.forecast.parts[i].condition = jo["forecast"]["parts"][i]["condition"].as<const char *>();
    weather.forecast.parts[i].daytime = jo["forecast"]["parts"][i]["daytime"].as<const char *>();
    ...
    weather.forecast.parts[i].icon = jo["forecast"]["parts"][i]["icon"].as<const char *>();
    weather.forecast.parts[i].part_name = jo["forecast"]["parts"][i]["part_name"].as<const char *>();
    ...
    weather.forecast.parts[i].wind_dir = jo["forecast"]["parts"][i]["wind_dir"].as<const char *>();
  ...
  weather.forecast.sunrise = jo["forecast"]["sunrise"].as<const char *>();
  weather.forecast.sunset = jo["forecast"]["sunset"].as<const char *>();
  ...
  weather.info.url = jo["info"]["url"].as<const char *>();
  weather.now = jo["now"].as<int>();
  weather.now_dt = jo["now_dt"].as<const char *>();
#if PRINT_DATA
  print_weather();
#endif
  return true;
